...
export default function AboutUs() {
  const professionalTraining = [
    {
      ...
      icon: (
        <svg className="program-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: 15, height: 15, color: 'var(--gold)', flexShrink: 0 }}>
  ...
  return (
    <div className="page-container about-page-container" style={{
      ...
      <style>{`
        #app-container:has(.about-page-container) + footer {
          margin-top: 0 !important;
        }
      `}</style>
      {/* Hero */}
      <div className="cinematic-hero stagger-1">
        <img src="https://images.unsplash.com/photo-1528164344705-47542687000d?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80" alt="Dojo Interior" />
        <div className="cinematic-overlay">
          <h1 className="page-title slide-left" style={{ marginBottom: 0 }}>The Spirit of Karate</h1>
          <p className="slide-right">Forging Character. Building Strength.</p>
        </div>
      </div>
          ...
          <div className="about-content-wrapper">
              ...
              <div className="about-lines-container" style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
                {[
                  ...
                  <div key={sIdx} style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                    <h4 style={{ color: 'var(--gold)', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '0.2rem', fontWeight: '800' }}>
                      {sec.section}
                    </h4>
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                      {sec.lines.map((line, lIdx) => (
                        <li key={lIdx} style={{ fontSize: '0.98rem', color: 'var(--text-2)', display: 'flex', alignItems: 'flex-start', gap: '0.55rem', lineHeight: '1.4' }}>
                          <span style={{ color: 'var(--gold)', marginTop: '0.25rem', fontSize: '0.75rem' }}>⚡</span>
                          <span>{line}</span>
                        </li>
                      ))}
                    </ul>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '1.25rem', marginTop: '2.5rem' }}>
              <div style={{ background: 'rgba(0, 0, 0, 0.25)', padding: '1.25rem', borderRadius: '10px', border: '1px solid rgba(255, 255, 255, 0.03)', textAlign: 'center' }}>
                <div style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--gold)', marginBottom: '0.25rem' }}>40+</div>
                <div style={{ fontSize: '0.78rem', color: 'var(--text-2)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Years Experience</div>
              </div>
              <div style={{ background: 'rgba(0, 0, 0, 0.25)', padding: '1.25rem', borderRadius: '10px', border: '1px solid rgba(255, 255, 255, 0.03)', textAlign: 'center' }}>
                <div style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--gold)', marginBottom: '0.25rem' }}>1000+</div>
                <div style={{ fontSize: '0.78rem', color: 'var(--text-2)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Black Belts Certified</div>
              </div>
            </div>
          </div>
        </div>
              ...
              {[
                ...
                <div key={idx} className="kun-scroll-item" style={{ alignItems: 'flex-start', gap: '0.85rem' }}>
                  ...
                  <div className="kun-text-container" style={{ flex: 1 }}>
                    ...
                    <span className="kun-rule-detail" style={{ fontSize: '0.76rem', color: 'var(--text-2)', lineHeight: 1.4, marginTop: '0.15rem', display: 'block' }}>{rule.detail}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {[
            ...
            <div key={name} className="card glass-glow instructor-card" style={{ textAlign: 'center' }}>
              ...
              <div style={{ position: 'relative', zIndex: 2 }}>
                <div style={{ width: 90, height: 90, borderRadius: '50%', background: '#111', margin: '0 auto 1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2.25rem', border: '2px solid var(--gold)' }}>👤</div>
                <h3 style={{ marginBottom: '0.35rem' }}>{name}</h3>
                <p className="text-gold" style={{ fontSize: '0.82rem', marginBottom: '0.65rem' }}>{role}</p>
                <p style={{ color: 'var(--text-2)', fontSize: '0.85rem', lineHeight: 1.6 }}>{bio}</p>
              </div>
            </div>
          ))}
        </div>
